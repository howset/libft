...
# ---------
# Variables. build the objects created
OBJ	= $(SRC:.c=.o)

# ---------
# Variables. set the name of the final binary using NAME, so the AutoGrader can find the binary correctly.
NAME = libft.a
...
# The rule for building a .o file from a .c file is called the .c.o (pronounced "dot-see-dot-oh") suffix rule.
# ---------
# https://hackernoon.com/how-to-create-a-library-in-c-with-a-makefile
# the “ar” command creates an archive file, which becomes our static library. The r flag to replace/add files to the archive, c to create if it doesn’t exist, s write an index into the archive for faster searches (if the library contains many object files).
# run by default because placed first
$(NAME):	$(OBJ)
	ar rcs $(NAME) $(OBJ)
